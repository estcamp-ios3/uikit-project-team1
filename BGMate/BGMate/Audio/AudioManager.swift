...
enum AudioError: Error {
    ...
    case playerInitFailed(underlying: Error)
}
...
class AudioManager: NSObject, AVAudioPlayerDelegate {
    ...
    // 오디오 파일 로드 및 재생 준비 (던지는 버전)
    func prepareAudioThrowing(named name: String, fileExtension: String) throws {
        guard let url = Bundle.main.url(forResource: name, withExtension: fileExtension) else {
            throw AudioError.fileNotFound(name: name, ext: fileExtension)
        }
        do {
            ...
            throw AudioError.playerInitFailed(underlying: error)
        }
    }

    // 오디오 파일 로드 및 재생 준비 (기존 래퍼 - 호출부 호환 유지)
    func prepareAudio(named name: String, fileExtension: String) {
        do {
            try prepareAudioThrowing(named: name, fileExtension: fileExtension)
        } catch {
    ...
    // MARK: - 오디오 세션 구성
    /// 백그라운드 및 무음모드에서도 재생될 수 있도록 AVAudioSession을 설정합니다.
    func configureAudioSession() {
        let session = AVAudioSession.sharedInstance()
        do {
            // .playback 카테고리는 무음모드 스위치를 무시하고 계속 소리를 재생합니다.
